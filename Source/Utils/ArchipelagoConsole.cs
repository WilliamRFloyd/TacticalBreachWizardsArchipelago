...
public static class ArchipelagoConsole
{
    ...
    public static void OnGUI()
    {
        ...
        if (!CommandText.IsNullOrWhiteSpace() && GUI.Button(SendCommandButton, "Send"))
        {
            ...
            {
                long id = long.Parse(CommandText.Substring(1));
                ArchipelagoClient.ServerData.CheckedLocations.Add(id);
                ArchipelagoClient.SendChecks();
                ArchipelagoConsole.LogMessage($"Sent check for location {id} to server.");
            }
            CommandText = "";
        }
    }
